...
def load_data(job_description_path, cv_folder_path):
    # Load job description with UTF-8 encoding and error handling
    ...
    # Load candidate CVs with UTF-8 encoding and error handling
    candidate_cvs = {}
    for filename in os.listdir(cv_folder_path):
        if filename.endswith('.txt'):
            with open(os.path.join(cv_folder_path, filename), 'r', encoding='utf-8', errors='replace') as file:
...
if __name__ == "__main__":
    job_description_path = 'C:/Users/khush/OneDrive\Documents/ai_job/job_desc/job5.txt'  # Path to job description file
